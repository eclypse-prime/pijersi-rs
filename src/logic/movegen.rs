...
impl Board {
    ...
    pub fn available_player_actions(&self, current_player: Player) -> Actions {
        ...
        for index in 0..N_CELLS {
            ...
            if self.colour(current_player).get(index) {
                self.available_piece_actions(index, &mut player_actions);
            }
        }
        player_actions
    }
    ...
    #[inline]
    pub fn available_piece_actions(&self, index_start: CellIndex, player_actions: &mut Actions) {
        ...
        if piece_start.is_stack() {
            ...
            for index_mid in self.available_unstacks(index_start, piece_start) {
                player_actions.push(Action::from_indices(index_start, index_start, index_mid));
            }
        } else {
            for index_mid in self.available_stacks(index_start, piece_start) {
    ...
    fn available_piece_captures(&self, index_start: CellIndex, player_actions: &mut ActionsLight) {
        todo!()
    }
}
