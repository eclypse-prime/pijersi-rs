...
use crate::logic::{COLOUR_MASK, HALF_PIECE_WIDTH, INDEX_MASK, INDEX_WIDTH, TOP_MASK, TYPE_MASK, TYPE_WISE};
...
#[inline]
pub fn evaluate_action_terminal(cells: &[u8; 45], action: u64, current_player: u8, previous_score: i64, previous_piece_scores: &[i64; 45]) -> i64 {
        ...
        {
            if (current_player == 1 && (index_end <= 5)) || (current_player == 0 && (index_end >= 39))
            {
                return -MAX_SCORE;
            }
        }
        ...
        else
        {
            ...
            if mid_piece != 0 && (mid_piece & COLOUR_MASK) == (start_piece & COLOUR_MASK) && (index_mid != index_start)
            {
                end_piece = (start_piece & TOP_MASK) + (mid_piece << HALF_PIECE_WIDTH);
                start_piece = start_piece >> HALF_PIECE_WIDTH;
                mid_piece = 0;
            ...
            else if end_piece != 0 && (end_piece & COLOUR_MASK) == (start_piece & COLOUR_MASK)
            {
                ...
                mid_piece = mid_piece >> HALF_PIECE_WIDTH;
        ...
        if current_player == 0 {current_score} else {-current_score}
    }